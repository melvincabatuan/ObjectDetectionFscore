...
void writeImageSeq(Mat src, int counter, string folderName){
...
int main( int argc, const char** argv )
{
	...
	cout << "This is software for performing a simple detection using a trained cascade classifier model." << endl;
        cout << "USAGE ./detect_simple -model <model.xml> -annotations <annotations.txt> -detections <output.txt> -folder <folderName> -box <0 or 1>" << endl;
        return 0;
    }
    ...
    for(int i = 0; i < (int)groundtruth.size(); i++){
        ...
        //detector.detectMultiScale(equalize, outputs, 1.05, 3, 0, Size(48, 48));  // Build 2 best for LBP cascade
        detector.detectMultiScale(equalize, outputs, 1.05, 9, 0, Size(48, 48));
        ...
        for(int rectIdx = 0; rectIdx < (int)outputs.size(); rectIdx++){
            ...
            for (int annoIdx = 0; annoIdx < (int)current_regions.size(); annoIdx++){
             ...
             if (false_positive){
                   ...
                   //writeImageSeq(fp_roi, FP, folderName);
             } 
        ...
        for (int annoIdx = 0; annoIdx < (int)current_regions.size(); annoIdx++){
                ...
                if (no_matches) {
                    FN++;
                    //cout <<"\n False negative image "<< FN <<": "<< groundtruth[i].filename << endl;
                }
